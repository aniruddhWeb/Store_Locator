...
import s from './ProductMap.module.css';
import {
...
export const ProductMap = React.memo(
  ({ product, standalone, showClaim }: ProductMapProps) => {
        ...
        ((product?.businessProductReseller || []) as Business[])
          .filter(
            item =>
              ...
              !!item?.bizClaim ||
              !!item?.bizClaimUnblurred,
          )
          .map(businessItem => {
            if (businessItem.bizClaim || businessItem.bizClaimUnblurred) {
              return {
                business: businessItem,
                product,
              };
            }
            const productItem = product?.productReseller?.find(
              resellerProduct =>
                resellerProduct?.business.some(
                  subItem =>
                    subItem.bizBusinessID === businessItem.bizBusinessID,
                ),
            );
            if (productItem) {
              return {
